CREATE OR REPLACE PROCEDURE DAAS_RAW_EXACT_TARGET.ET_BOUNCES_EVENT_RAW_LOAD_PROC(BATCH_ID FLOAT, SHARD_NAME VARCHAR, WAREHOUSE_NAME VARCHAR, CUSTOM_PARAM1 VARCHAR, CUSTOM_PARAM2 VARCHAR) 
RETURNS VARCHAR
LANGUAGE JAVASCRIPT 
...
#####################################################################################
Author: Divya
Purpose: Load data from DAAS_STG_EXACT_TARGET.ET_BOUNCES_EVENT_STG to DAAS_RAW_EXACT_TARGET.ET_BOUNCES_EVENT_RAW table
...
FROM
	DAAS_STG_EXACT_TARGET.ET_BOUNCES_EVENT_STG_STREAM STG 
WHERE 
	STG.METADATA$ACTION = 'INSERT'
`;   

my_sql_command_2 =`
INSERT INTO DAAS_RAW_EXACT_TARGET.ET_BOUNCES_EVENT_RAW
(
	I_CLIENTID,                
	I_SENDID,                  
	C_SUBSCRIBER_KEY,          
...
my_sql_command_3 = `SELECT COUNT(*) AS raw_insert_count FROM DAAS_RAW_EXACT_TARGET.ET_BOUNCES_EVENT_RAW WHERE BATCH_ID = ` +BATCH_ID;
