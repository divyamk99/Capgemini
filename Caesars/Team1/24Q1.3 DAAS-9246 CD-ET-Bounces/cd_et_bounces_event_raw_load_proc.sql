CREATE OR REPLACE PROCEDURE DAAS_RAW_EXACT_TARGET.CD_ET_BOUNCES_EVENT_RAW_LOAD_PROC(BATCH_ID FLOAT, SHARD_NAME VARCHAR, WAREHOUSE_NAME VARCHAR, CUSTOM_PARAM1 VARCHAR, CUSTOM_PARAM2 VARCHAR) 
RETURNS VARCHAR
LANGUAGE JAVASCRIPT 
...
#####################################################################################
Author: Divya
Purpose: Load data from DAAS_STG_EXACT_TARGET.CD_ET_BOUNCES_EVENT_STG to DAAS_RAW_EXACT_TARGET.CD_ET_BOUNCES_EVENT_RAW table
...
my_sql_command_1 = `
CREATE OR REPLACE TABLE DAAS_TEMP.CD_ET_BOUNCES_EVENT_RAW_TEMP AS
SELECT 
	STG.I_CLIENTID AS I_CLIENTID,
	STG.I_SENDID AS I_SENDID,
	STG.C_SUBSCRIBER_KEY AS C_SUBSCRIBER_KEY,
	COALESCE(DAAS_COMMON.EXACT_TARGET_I_DMID_UDF(STG.C_SUBSCRIBER_KEY),0) AS I_DMID,
...
FROM
	DAAS_STG_EXACT_TARGET.CD_ET_BOUNCES_EVENT_STG_STREAM STG 
WHERE 
...
SELECT
	...
	SRC.MD5_CHECKSUM
FROM	
	DAAS_TEMP.CD_ET_BOUNCES_EVENT_RAW_TEMP SRC
WHERE NOT EXISTS
(
	SELECT 
		1
	FROM
		DAAS_RAW_EXACT_TARGET.CD_ET_BOUNCES_EVENT_RAW RAW
	WHERE
...
my_sql_command_3 = `SELECT COUNT(*) AS raw_insert_count FROM DAAS_RAW_EXACT_TARGET.CD_ET_BOUNCES_EVENT_RAW WHERE BATCH_ID = ` +BATCH_ID;

my_sql_command_4 = `SELECT COUNT(*) AS source_count FROM DAAS_TEMP.CD_ET_BOUNCES_EVENT_RAW_TEMP`;	
