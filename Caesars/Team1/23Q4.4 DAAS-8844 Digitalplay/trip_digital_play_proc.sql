...
Version: 1.0
Version: 1.1 - 11/20/2023 Adam Tobin - Changing from CTEs to temp tables to reduce memory spillage.
Version: 1.2 - Added delete statement to remove 0 metrics 
               Capturing metrics for Digital and B&M merge
Updated By: Divya M K
Version: 2.1(Added Merge statement for TEMP table and Delete processed record from TEMP table)
#####################################################################################
*/
...
my_sql_command_1_a = `
MERGE INTO DAAS_TEMP.DAILY_ACTIVITY_SUMMARY_DIGITAL_PLAY_TEMP_DTF TGT
USING (SELECT 
	DGXBL.GUEST_UNIQUE_ID AS GUEST_UNIQUE_ID,
   DATE(DTF_STREAM.REPORT_DT) AS GUEST_ACTIVITY_DATE,
   CID,
   DTF_STREAM.SOURCE_SYSTEM_NM AS SOURCE_SYSTEM_NM,
   BRAND
FROM 
   (
     SELECT CID,REPORT_DT,SOURCE_SYSTEM_NM,BRAND FROM DAAS_CORE.DIGITAL_TRANSACTIONS_FACT_TRIP_STREAM WHERE METADATA$ACTION = 'INSERT'
     UNION
     SELECT CID,GUEST_ACTIVITY_DATE,SOURCE_SYSTEM_NM,BRAND FROM DAAS_TEMP.DAILY_ACTIVITY_SUMMARY_DIGITAL_PLAY_TEMP_DTF
   )
     DTF_STREAM
LEFT JOIN 
...
ON TGT.GUEST_ACTIVITY_DATE=DTF.GUEST_ACTIVITY_DATE
AND TGT.CID=DTF.CID
AND TGT.SOURCE_SYSTEM_NM=DTF.SOURCE_SYSTEM_NM
AND TGT.BRAND=DTF.BRAND
WHEN MATCHED THEN 
UPDATE SET 
	GUEST_UNIQUE_ID = DTF.GUEST_UNIQUE_ID
...
(
DTF.GUEST_UNIQUE_ID,
DTF.GUEST_ACTIVITY_DATE,
DTF.CID,
DTF.SOURCE_SYSTEM_NM,
DTF.BRAND
);
`;
...
JOIN 
    (SELECT DISTINCT GUEST_UNIQUE_ID,GUEST_ACTIVITY_DATE FROM DAAS_TEMP.DAILY_ACTIVITY_SUMMARY_DIGITAL_PLAY_TEMP_DTF WHERE GUEST_UNIQUE_ID IS NOT NULL)DTF_STREAM
ON 
...
my_sql_command_1_b_1 = `DELETE FROM DAAS_TEMP.DAILY_ACTIVITY_SUMMARY_DIGITAL_PLAY_TEMP_DTF WHERE GUEST_UNIQUE_ID IS NOT NULL;`;

/* delete the records where all the metrics are 0 */
my_sql_command_1_1 = ` 
DELETE FROM
    DAAS_TEMP.DAILY_ACTIVITY_SUMMARY_DIGITAL_PLAY_TEMP_DIGITAL_SUMMARY 
WHERE 
  (CASINO_GGR+SPORTS_GGR+POKER_GGR+SPORTS_THEO+CASINO_NET_THEO+CASINO_WAGER+SPORTS_WAGER+CASINO_BONUS+SPORTS_BONUS) = 0
`;
...
/* Digital Source*/
digital_source_count_command =`SELECT COUNT(*) FROM DAAS_TEMP.DAILY_ACTIVITY_SUMMARY_DIGITAL_PLAY_TEMP_DIGITAL_SUMMARY`;

/* B&M Source*/
bmtrip_source_count_command =`SELECT COUNT(*) FROM DAAS_TEMP.DAILY_ACTIVITY_SUMMARY_TRIP`;

target_count_insert_command =`SELECT COUNT(*) FROM DAAS_CORE.DAILY_ACTIVITY_SUMMARY WHERE BATCH_ID = `+BATCH_ID+` AND LAST_DML_CD = 'I'`; 

target_count_update_command =`SELECT COUNT(*) FROM DAAS_CORE.DAILY_ACTIVITY_SUMMARY WHERE BATCH_ID = `+BATCH_ID+` AND LAST_DML_CD = 'U'`; 
...
{ 
	...
	
	{
		...
		snowflake.execute({sqlText: my_sql_command_1_b_1});
		snowflake.execute({sqlText: my_sql_command_1_1});
		...
		digital_source_count_statment = snowflake.execute({sqlText: digital_source_count_command } );
	    digital_source_count_statment.next();
	    digital_source_count_result = digital_source_count_statment.getColumnValue(1);
		
		my_sql_command = "CALL DAAS_COMMON.BATCH_CONTROL_UPDATE_BATCH_METRICS_PROC(" + BATCH_ID + ", '" + SHARD_NAME + "', 'Digital Source Count', '" + digital_source_count_result + "')";
	    sf_stmt = snowflake.execute( {sqlText: my_sql_command });
	    sf_stmt.next();	
	...
	
    {
	    ...
	    bmtrip_source_count_statment = snowflake.execute({sqlText: bmtrip_source_count_command } );
	    bmtrip_source_count_statment.next();
	    bmtrip_source_count_result = bmtrip_source_count_statment.getColumnValue(1);
		
		
		target_count_insert_statement = snowflake.execute({sqlText: target_count_insert_command } );
	    target_count_insert_statement.next();
	    target_count_insert_result = target_count_insert_statement.getColumnValue(1);
		
		
		target_count_update_statement = snowflake.execute({sqlText: target_count_update_command } );
	    target_count_update_statement.next();
	    target_count_update_result = target_count_update_statement.getColumnValue(1);
		
		
	    
		my_sql_command = "CALL DAAS_COMMON.BATCH_CONTROL_UPDATE_BATCH_METRICS_PROC(" + BATCH_ID + ", '" + SHARD_NAME + "', 'B&M Source Count', '" + bmtrip_source_count_result + "')";
	    sf_stmt = snowflake.execute( {sqlText: my_sql_command });
	    sf_stmt.next();	
	
	
	    my_sql_command = "CALL DAAS_COMMON.BATCH_CONTROL_UPDATE_BATCH_METRICS_PROC(" + BATCH_ID + ", '" + SHARD_NAME + "', 'Digital Play - Daily Activity Summary Insert Count', '" + target_count_insert_result + "')";
	    sf_stmt = snowflake.execute( {sqlText: my_sql_command });
	    sf_stmt.next();
		
		my_sql_command = "CALL DAAS_COMMON.BATCH_CONTROL_UPDATE_BATCH_METRICS_PROC(" + BATCH_ID + ", '" + SHARD_NAME + "', 'Digital Play - Daily Activity Summary Update Count', '" + target_count_update_result + "')";
	    sf_stmt = snowflake.execute( {sqlText: my_sql_command });
	    sf_stmt.next();
