...
-- Step 4) Take backup of Trip tables [Thru Standardized Proc - CLONE_TABLE_PROC]

CALL DAAS_SECURITY.CLONE_TABLE_PROC('DAAS_UAT', 'DAAS_UAT', 'DAAS_CORE', 'DAILY_ACTIVITY_SUMMARY', NULL ,NULL);
CALL DAAS_SECURITY.CLONE_TABLE_PROC('DAAS_UAT', 'DAAS_UAT', 'DAAS_CORE', 'TRIP_SUMMARY', NULL ,NULL);
CALL DAAS_SECURITY.CLONE_TABLE_PROC('DAAS_UAT', 'DAAS_UAT', 'DAAS_CORE', 'TRIP_DETAIL', NULL ,NULL);
CALL DAAS_SECURITY.CLONE_TABLE_PROC('DAAS_UAT', 'DAAS_UAT', 'DAAS_CORE', 'TRIP_MASTER', NULL ,NULL);
CALL DAAS_SECURITY.CLONE_TABLE_PROC('DAAS_UAT', 'DAAS_UAT', 'DAAS_CORE', 'TRIP_MERGE_PROCESSED', NULL ,NULL);
CALL DAAS_SECURITY.CLONE_TABLE_PROC('DAAS_UAT', 'DAAS_UAT', 'DAAS_CORE', 'TRIP_MERGE_QUEUE', NULL ,NULL);
CALL DAAS_SECURITY.CLONE_TABLE_PROC('DAAS_UAT', 'DAAS_UAT', 'DAAS_CORE', 'TRIP_RECALC_PROCESSED', NULL ,NULL);
CALL DAAS_SECURITY.CLONE_TABLE_PROC('DAAS_UAT', 'DAAS_UAT', 'DAAS_CORE', 'TRIP_RECALC_QUEUE', NULL ,NULL);

-- Step 5) Creating table to capture the Hotel Extra SKs
...
--- Step 7) If the count is huge, divide the records into chunks
...
-- Step 8) Run the Adhoc scripts to soft delete the extra SKs and recalculate the trips 
...
-- Step 9) Execute OnDemand tasks from Recalc to Trip Summary to cleanup these SKs [Thru OnDemand Tasks]
...
--- Step 11) RESUME The TRIP_ROOT_TASK 

ALTER TASK DAAS_COMMON.TRIP_ROOT_TASK RESUME;

-- Step 12) Revert back the warehouse of the trip jobs to l warehouse

CALL DAAS_ADHOC.ADHOC_SQL_EXECUTE('UPDATE DAAS_COMMON.JOB
SET WAREHOUSE_NAME = \'DAAS_PROD_IT_WHSRCTORAW_COMPUTE\'
WHERE JOB_ID IN (731, 525, 637, 638, 657, 656, 654, 653, 523, 526, 527, 532, 528, 529, 655');

-- Step 13) Revert back the Task time to 2 Hours
