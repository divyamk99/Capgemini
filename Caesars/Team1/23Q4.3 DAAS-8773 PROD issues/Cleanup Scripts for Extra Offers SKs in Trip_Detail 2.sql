...
-- Step 4) Execute TRIP_ROOT_TASK to consume data from the stream [Thru Aggregared Role][Only for PROD]

EXECUTE TASK DAAS_COMMON.TRIP_ROOT_TASK;


-- Step 5) Take backup of Trip tables [Thru Standardized Proc - BACKUP_TABLE_PROC]
...
-- Step 6) Creating table to capture the Offers Extra SKs

CALL DAAS_ADHOC.ADHOC_SQL_EXECUTE('CREATE OR REPLACE TABLE DAAS_TEMP.OFFERS_EXTRA_SKS_CLEANUP_TRIPS AS
...
-- Step 7) Run the count query to check if the cleanup needs to be performed in chunks
...
-- Step 8) If the count is huge, divide the records into chunks
...
JOIN (SELECT 
	...
	FROM
		DAAS_CORE.TRIP_MASTER TM
	WHERE 
		TM.TRIP_MASTER_ID IN (SELECT DISTINCT TRIP_MASTER_ID FROM DAAS_TEMP.OFFERS_EXTRA_SKS_CLEANUP_TRIPS WHERE RANK<=?)
	AND TM.DELETE_IND = 'N') C
ON
		A.TRIP_MASTER_ID = C.TRIP_MASTER_ID
WHERE
...
-- Step 9) Run the Adhoc scripts to soft delete the extra SKs and recalculate the trips 
...
-- Step 10) Execute OnDemand tasks from Recalc to Trip Summary sequentially to cleanup these SKs [Thru OnDemand Tasks]
...
--Repeat the step 7,8 & 9 for all the chunks


-- Step 11) Ensure there is no more discrepency (Validation)--It should result 0
...
--Step 12) Resume the suspended task 

ALTER TASK DAAS_COMMON.RAW2CORE_PDB_OFFER_LEDGER_FACT_TASK RESUME;


-- Step 13) Revert back the warehouse of the trip jobs to l warehouse
...
-- Step 14) Revert back the Task time to 2 Hours
