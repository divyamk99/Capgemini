CREATE OR REPLACE PROCEDURE DAAS_ADHOC.FREEPLAY_METRICMISMATCH_ADHOC_PROC()
...
/*TS PROP VS ENT */
	...
	snowflake.execute( {sqlText: `CREATE OR REPLACE TABLE DAAS_TEMP.FREEPLAY_PROP_ENT_TS_DESCRIPANCY
									AS
									WITH FREEPLAY AS
(
WITH ABS_LOGIC AS
...
OR PROPERTY_DLC_FSP_DOLLAR_AMT <> ENTERPRISE_DLC_FSP_DOLLAR_AMT;` } );

/*TS MARK VS ENT*/ 

	snowflake.execute( {sqlText: `CREATE OR REPLACE TABLE DAAS_TEMP.FREEPLAY_MARK_ENT_TS_DESCRIPANCY
									AS
									WITH FREEPLAY AS
(
WITH ABS_LOGIC AS
...
/*DAS PROP VS ENT*/ 
	...
	snowflake.execute( {sqlText: `CREATE OR REPLACE TABLE DAAS_TEMP.FREEPLAY_PROP_ENT_DAS_DESCRIPANCY
									AS
										WITH FREEPLAY AS(
	...
	SELECT 
		...
		ABS(SUM(CASE WHEN TRIP_TYPE = 'PROPERTY' THEN  DLC_FSP_POINT_AMT*PROPERTY_SIGN ELSE 0 END)) AS PROPERTY_DLC_FSP_POINT_AMT,
		ABS(SUM(CASE WHEN TRIP_TYPE = 'ENTERPRISE' THEN  DLC_FSP_POINT_AMT*ENTERPRISE_SIGN ELSE 0 END)) AS ENTERPRISE_DLC_FSP_POINT_AMT,
		ABS(SUM(CASE WHEN TRIP_TYPE = 'PROPERTY' THEN  DLC_FSP_DOLLAR_AMT*PROPERTY_SIGN ELSE 0 END)) AS PROPERTY_DLC_FSP_DOLLAR_AMT,
	...
	GROUP BY 
		...
		DAS.GUEST_ACTIVITY_DATE
	HAVING 
	PROPERTY_DLC_FSP_POINT_AMT<>ENTERPRISE_DLC_FSP_POINT_AMT
	OR 
	PROPERTY_DLC_FSP_DOLLAR_AMT<>ENTERPRISE_DLC_FSP_DOLLAR_AMT)
	SELECT DISTINCT GUEST_UNIQUE_ID
	FROM FREEPLAY;` } );
